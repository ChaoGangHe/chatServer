...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        cerr << "command invalid! exmaple: ./ChatClient 127.0.0.1 6000" << endl;
        exit(-1);
    }
    ...
    for (;;)
    {
        ...
        case 2: // register业务
        {
            ...
            else
            {
                ...
                {
                    json resonsejs = json::parse(buffer);
                    if (0 != resonsejs["errno"].get<int>()) // 注册失败
                    {
                        cerr << name << "is already exist,register error!" << endl;
                    }
                    else
                    {
                        cout << name << "register success,userid is" << resonsejs["id"] << ",do not forget it!" << endl;
                    }
                }
            }
        }
...
void showCurrentUserData()
{
    ...
    if (!g_currentUserFriendList.empty())
    {
        ...
        {
            cout << user.getId() << " " << user.getName() << " " << user.getSate() << endl;
        }
    }
    ...
    if (!g_currentUserGroupList.empty())
    {
        for (Group &group : g_currentUserGroupList)
        {
            ...
            {
                cout << user.getId() << " " << user.getName() << " " << user.getSate()
                     << " " << user.getRole() << endl;
            }
        }
    }
